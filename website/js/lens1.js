const Lens1 = (() => {
  ...
  function drawChoropleth(districts, geoData) {
    ...
    // Filter out the large rural zone for better urban zoom
    const urbanFeatures = geoData.features.filter(f => {
      const n = getName(f).toLowerCase();
      return !n.includes('zone') && !n.includes('forain');
    });
    const urbanGeo = { type: 'FeatureCollection', features: urbanFeatures };

    const projection = d3.geoIdentity().reflectY(true).fitExtent([[20, 20], [W-20, H-50]], urbanGeo);
    ...
    g.selectAll('text.district-label')
      ...
      .text(f => { const n = getName(f); if(n.toLowerCase().includes('zone')) return 'Zones for.'; return n.split('/')[0].split('–')[0].trim().substring(0,10); });
