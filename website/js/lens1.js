const Lens1 = (() => {
  ...
  function drawStackedArea(cityPop, allPop) {
    ...
    const distLine = g.append('path')
      .attr('class','dist-overlay')
      .attr('fill','none')
      .attr('stroke','#fbbf24')
  ...
  function drawPyramid(popAge, year) {
    ...
    groups.forEach((g,i)=>{
      const y0 = 10+i*rowH;
      const bh = rowH*.72;
      const bwM = (g.male/yd.pop_total)*barScale;
      const bwF = (g.female/yd.pop_total)*barScale;

      svg.append('rect')
        .attr('x',cx-LABEL_W/2-2-bwM)
        .attr('y',y0)
        .attr('width',bwM)
        .attr('height',bh)
        .attr('fill','#1d4ed8')
  ...
  function drawChoropleth(districts, geoData) {
    ...
    function addControl(label, title) {
    ...
    const zoomInBtn = addControl('+', 'Zoom in');
    const zoomOutBtn = addControl('−', 'Zoom out');
    const resetZoomBtn = addControl('reset', 'Reset zoom');
    ...
    function updateLabelScale(k) {
      ...
      labelSelection.attr('transform', f => {
    ...
    const paths = g.selectAll('path.district')
      ...
      .attr('fill', f => {
        ...
        return match ? colorScale(match.pct_foreign || 0) : '#dbeafe';
      })
      .attr('stroke','white')
      .attr('stroke-width',1)
      .attr('opacity',0)
      ...
      .on('mouseenter', function(event, f) {
        ...
        if (match) {
          TT.show(
            ...
            <em style="color:#9ca3af;font-size:10px">Click to filter all charts</em>`,
            event
          );
        } else {
    ...
    labelSelection = g.selectAll('text.district-label')
      ...
      .attr('font-size', f => {
        ...
        if (isMobile) return n.includes('zone') || n.includes('forain') ? 8 : 7;
        return n.includes('zone') || n.includes('forain') ? 9 : 11;
      })
      ...
      .text(f => {
        ...
        if (low.includes('zone') || low.includes('forain')) return 'Zones foraines';
    ...
    filter.onChange(districtName => {
      ...
      g.selectAll('path.district')
        ...
        .attr('stroke-width', f => {
          const match = findMatch(getName(f));
          return match && match.district === districtName ? 2.6 : 1;
        });
    });
  }
