const Lens2 = (() => {
  ...
  function drawDualLine(cityHH, allHH){
    ...
    const yL=d3.scaleLinear().domain([d3.min(data,d=>d.pct_1person)*.92,d3.max(data,d=>d.pct_1person)*1.04]).range([h,0]);
    ...
    // District overlay
    const distPath = g.append('path').attr('class','dist-hh').attr('fill','none')
      .attr('stroke','#fbbf24').attr('stroke-width',2).attr('stroke-dasharray','5,3').attr('opacity',0);
    ...
    // District title label
    const distLabel = g.append('text').attr('x',w-4).attr('y',8).attr('text-anchor','end')
      .attr('font-size',10).attr('fill','#f59e0b').attr('font-weight','600').attr('opacity',0);
    ...
    window.DistrictFilter.onChange(districtName => {
      if (!districtName) {
        distPath.attr('opacity', 0);
        distLabel.attr('opacity', 0);
        return;
      }
      const distData = allHH.filter(d => d.district === districtName).sort((a,b)=>a.year-b.year);
      if (!distData.length) return;
      const lineFn = d3.line().x(d=>x(d.year)).y(d=>yL(d.pct_1person)).curve(d3.curveCatmullRom);
      distPath.datum(distData).attr('d', lineFn).attr('opacity', 1);
      distLabel.text(districtName.replace(/^\d+\s*[-–]\s*/,'')).attr('opacity',1);
    });
  }
